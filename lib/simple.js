...
pack: "𝙳𝚁𝙰𝙲𝚄𝙻𝙰 𝙼𝙳", // The pack name
author: "𝙳𝚁𝙰𝙲𝚄𝙻𝙰 𝙼𝙳", // The author name
...
let stok = getRandomFile(".webp")
  ...
  title: `${botName}`,body: `${baileysVersion}`,previewType:"PHOTO",thumbnailUrl: 'https://files.catbox.moe/1urvk8.jpg',
  sourceUrl:`${web}`	
...
return [h + ' Hours ', m + ' Minutes ', s + ' Seconds'].map(v => v.toString().padStart(2, 0)).join(' ')
}
...
conn.msToMinute = (ms) => {
let m = isNaN(ms) ? '--' : Math.floor(ms / 60000) % 60
return [m + ' Minutes '].map(v => v.toString().padStart(2, 0)).join(' ')
}
conn.msToSecond = (ms) => {
let s = isNaN(ms) ? '--' : Math.floor(ms / 1000) % 60
return [s + ' Seconds'].map(v => v.toString().padStart(2, 0)).join(' ')
}
...
"url": `https://whatsapp.com/channel/0029Vap32qXLikg91i4TmH3K`,
"productImageCount": 1,
